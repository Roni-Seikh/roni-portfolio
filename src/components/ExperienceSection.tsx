  ...
  return (
    ...
    >
      ...
      <div className="max-w-4xl mx-auto w-full relative z-10">
              ...
              >
                ...
                <div className="ml-14 md:ml-12 pl-2">
                  <div className="md:hidden mb-1.5">
                    <span className="text-[10px] font-mono tracking-[0.2em] text-[#D4AF37]">{stop.dateRange}</span>
                  </div>

                  <h3
                    className="text-2xl sm:text-3xl tracking-wide text-white group-hover:text-[#F7E7C4] transition-colors mb-1 leading-none uppercase"
                    style={{ fontFamily: "'Bebas Neue', sans-serif" }}
                  >
                    {stop.title}
                  </h3>

                  <span className="block text-[10px] font-medium tracking-[0.2em] uppercase text-[#8C6D4F] mb-2" style={{ fontFamily: "'Montserrat', sans-serif" }}>
                    {stop.organization}
                  </span>

                  <p className="text-xs sm:text-[13px] font-light text-[#A8988B] leading-[1.7] max-w-lg group-hover:text-[#D5CBC0] transition-colors" style={{ fontFamily: "'Montserrat', sans-serif" }}>
                    {stop.description}
                  </p>

                  {stop.refId && (
                    <span className="block mt-2 text-[10px] font-mono text-[#6B5A48]">{stop.refId}</span>
                  )}
                </div>
              </motion.div>
            ))}
        ...
        {/* Education */}
        <div className="mt-16 pt-12 border-t border-[#8C6D4F]/20">
          <span className="block text-[10px] font-mono tracking-[0.25em] uppercase text-[#8C6D4F] mb-5">// EDUCATION</span>
          <div className="space-y-4">
            {education.map((ed) => (
              <div key={ed.school} className="flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-1 border-b border-[#8C6D4F]/10 pb-4">
                <div>
                  <span className="text-white text-sm font-medium" style={{ fontFamily: "'Montserrat', sans-serif" }}>{ed.school}</span>
                  <span className="block text-[12px] text-[#A8988B] font-light mt-0.5" style={{ fontFamily: "'Montserrat', sans-serif" }}>{ed.detail}</span>
                </div>
                <span className="text-[11px] font-mono text-[#8C6D4F] shrink-0">{ed.years}</span>
              </div>
            ))}
          </div>
        </div>
