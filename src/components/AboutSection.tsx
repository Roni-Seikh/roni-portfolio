...
import { profile, stats, achievements } from '../data/portfolioData';
...
export const AboutSection: React.FC = () => {
  ...
  const rotateX = useSpring(useTransform(mouseY, [-0.5, 0.5], [12, -12]), { damping: 18, stiffness: 220 });
  const rotateY = useSpring(useTransform(mouseX, [-0.5, 0.5], [-12, 12]), { damping: 18, stiffness: 220 });

  const spotlightBg = useTransform(
    [spotlightX, spotlightY],
    ([x, y]) => `radial-gradient(circle 240px at ${x}px ${y}px, rgba(255,255,255,0.25), rgba(212,175,55,0.14), transparent 80%)`
  );
    ...
    >
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          >
            ...
            <motion.p
              ...
              className="text-xs sm:text-sm md:text-[14.5px] font-light text-[#B3A497] leading-[1.85] tracking-wide mb-4 max-w-xl"
              style={{ fontFamily: "'Montserrat', sans-serif" }}
            >
              I'm <span className="text-[#F3DBB3] font-medium">{profile.name}</span>, a {profile.title.toLowerCase()} and Computer Science & Engineering student who builds software at the intersection of full-stack web development and applied AI/ML — from AI-powered developer tools to machine-learning dashboards.
            </motion.p>
            ...
            {/* Currently learning */}
            <motion.div variants={fadeUpVariants} className="flex flex-wrap items-center gap-2 mb-8 mt-2">
              <span className="text-[10px] font-mono tracking-[0.2em] uppercase text-[#8C6D4F] mr-1">Currently learning:</span>
              {profile.currentlyLearning.map((item) => (
                <span
                  key={item}
                  className="px-3 py-1 text-[10px] font-medium tracking-[0.14em] uppercase rounded-sm border border-[#D4AF37]/30 bg-[#16120E] text-[#F3DBB3]"
                >
                  {item}
                </span>
              ))}
            ...
            {/* Verified stats */}
            <motion.div variants={fadeUpVariants} className="grid grid-cols-2 sm:grid-cols-4 gap-6 pt-6 pb-2 border-t border-[#8C6D4F]/25">
              {stats.map((stat) => (
          ...
          <div className="lg:col-span-5 flex items-center justify-center relative perspective-[1400px]">
            ...
            >
              ...
              {/* Placeholder profile canvas — swap for a real photo any time */}
              {/* Profile photo */}
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1] }}
          className="grid grid-cols-1 sm:grid-cols-3 gap-5 mt-16 pt-12 border-t border-[#8C6D4F]/20"
        >
          {achievements.map((a) => (
            <div key={a.title} className="p-5 rounded-sm border border-[#8C6D4F]/25 bg-[#100D0B]/60">
              <h4 className="text-lg text-white mb-2 uppercase tracking-wide" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>
                {a.title}
              </h4>
              <p className="text-[12px] font-light text-[#A8988B] leading-relaxed" style={{ fontFamily: "'Montserrat', sans-serif" }}>
                {a.description}
              </p>
            </div>
          ))}
